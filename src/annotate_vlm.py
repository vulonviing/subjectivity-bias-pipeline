"""Step: annotate_vlm — VLM annotation of article main images via OpenAI Batch API.

Model: gpt-5.4-mini (snapshot pin: gpt-5.4-mini-2026-03-17, verified 2026-06-01)
    - Vision mode: image input via inline base64 data URL.
    - Endpoint: /v1/chat/completions (Batch API, Structured Outputs).
    - 50% cost discount vs synchronous API.

Image handling (CLAUDE.md Rule 6):
    - image_url is NEVER sent to the model.
    - Images are downloaded → resized to max 1024px → JPEG q85 → base64-encoded inline.
    - Cached under data/raw/images/<article_id>.jpg to avoid re-download on retry.

Workflow (three subcommands):
    submit  -- download images, build JSONL, upload to OpenAI Files, create batch, save state
    status  -- poll batch status, show request counts
    fetch   -- download completed output, join with input, write vlm_annotations.jsonl

Input : data/processed/articles_clean.jsonl (title + lead + image_url)
Output: data/processed/vlm_annotations.jsonl with fields:
    article_id, [all article metadata], vlm_label (OBJ|SUBJ), vlm_rationale, vlm_confidence,
    image_description, vlm_model, vlm_finish_reason

VLM labels are SILVER / REFERENCE annotations — never "gold".
"""
...
JPEG_QUALITY = 85
IMAGE_DETAIL = "auto"
DEFAULT_INPUT = "data/processed/articles_clean.jsonl"
DEFAULT_OUTPUT = "data/processed/vlm_annotations.jsonl"
DEFAULT_STATE = "data/processed/vlm_batch_state.json"
DEFAULT_PROMPT_PATH = "prompts/vlm_image_annotation.txt"
DEFAULT_SCHEMA_PATH = "prompts/vlm_response_schema.json"
IMAGE_CACHE_DIR = "data/raw/images"
BATCH_ENDPOINT = "/v1/chat/completions"
COMPLETION_WINDOW = "24h"
MAX_BATCH_BYTES = 180 * 1024 * 1024  # 180 MB hard abort (200 MB OpenAI limit)
...
# ---------------------------------------------------------------------------
# Shared helpers (mirrors annotate_llm.py)
# ---------------------------------------------------------------------------
...
def _load_state(state_path: str) -> dict:
    ...
    if not p.exists():
        sys.exit(
            ...
            "Run `python src/annotate_vlm.py submit` first to create a batch."
        )
...
def _download_and_preprocess_image(url: str, cache_path: Path) -> bytes:
    """Download image URL, resize to max 1024px, encode as JPEG q85. Cache on disk."""
    if cache_path.exists():
        return cache_path.read_bytes()

    user_agent = os.getenv("USER_AGENT", "media-bias-pipeline/1.0")
...
def cmd_submit(args: argparse.Namespace) -> None:
    ...
    for placeholder in ("{title}", "{lead}"):
        if placeholder not in user_template:
            sys.exit(
                f"User prompt template must contain the {placeholder} placeholder. "
                f"Check {args.prompt}."
            )

    log.info("Loading articles from %s", args.input)
    rows = list(_iter_jsonl(args.input))
    log.info("  %d articles loaded", len(rows))
    ...
    for row in rows:
        ...
        if not image_url:
            log.warning("No image_url for article_id=%s — skipping", article_id)
            skipped_no_image += 1
            continue

        cache_path = cache_dir / f"{article_id}.jpg"
        try:
            jpeg_bytes = _download_and_preprocess_image(image_url, cache_path)
        except Exception as exc:
            log.warning("Image download failed for article_id=%s: %s", article_id, exc)
            download_errors.append({"article_id": article_id, "image_url": image_url, "error": str(exc)})
        ...
        try:
            text_content = user_template.format(title=title, lead=lead)
        except KeyError as e:
            sys.exit(
                f"User template has unknown placeholder {e}. "
                f"Only {{title}} and {{lead}} are supported. Check {args.prompt}."
            )
    ...
    n = len(batch_lines)
    log.info(
        "Batch input: %d requests (skipped: %d no-image, %d download-error)",
        n, skipped_no_image, skipped_download_error,
    )

    if download_errors:
        err_path = Path(args.input).parent / "vlm_download_errors.jsonl"
        with open(err_path, "w", encoding="utf-8") as f:
            for e in download_errors:
                f.write(json.dumps(e, ensure_ascii=False) + "\n")
        log.warning("Download errors written to %s", err_path)

    batch_input_path = Path(args.input).parent / "vlm_batch_input.jsonl"
    with open(batch_input_path, "w", encoding="utf-8") as f:
        for line in batch_lines:
            f.write(json.dumps(line, ensure_ascii=False) + "\n")

    file_bytes = batch_input_path.stat().st_size
    log.info(
        "Batch JSONL written: %s (%d lines, %.1f MB)",
        batch_input_path, n, file_bytes / 1024 / 1024,
    )
    if file_bytes > MAX_BATCH_BYTES:
        sys.exit(
            f"Batch JSONL is {file_bytes/1024/1024:.1f} MB — exceeds 180 MB safety limit. "
            "Reduce image size or use --limit to split into smaller batches."
        )

    if args.dry_run:
        log.info("DRY RUN — first 3 batch lines (base64 truncated):")
        for line in batch_lines[:3]:
            preview = json.loads(json.dumps(line, ensure_ascii=False))
            for msg in preview["body"]["messages"]:
                if isinstance(msg.get("content"), list):
                    for part in msg["content"]:
                        if part.get("type") == "image_url":
                            url_val = part["image_url"]["url"]
                            part["image_url"]["url"] = url_val[:60] + "...[truncated]"
    ...
    batch = client.batches.create(
        ...
        metadata={"step": "annotate_vlm", "n": str(n), "model": model_display},
    )
    ...
    log.info("Next: `python src/annotate_vlm.py status` to poll, then `fetch` when completed.")
...
def cmd_status(args: argparse.Namespace) -> None:
    ...
    if batch.status == "completed":
        log.info("=> Ready to fetch. Run `python src/annotate_vlm.py fetch`.")
...
def cmd_fetch(args: argparse.Namespace) -> None:
    ...
    err_path = out_path.parent / "vlm_annotations.errors.jsonl"
    ...
    with open(out_path, "w", encoding="utf-8") as out_f, \
        ...
        for raw in raw_lines:
            ...
            out_row.update({
                "vlm_label": annotation.get("vlm_label"),
                "vlm_rationale": annotation.get("vlm_rationale"),
                "vlm_confidence": annotation.get("vlm_confidence"),
                "image_description": annotation.get("image_description"),
                "vlm_model": resolved_model,
                "vlm_finish_reason": finish_reason,
            })
            if args.keep_raw:
                out_row["vlm_raw_response"] = raw
    ...
    # Cost estimate (gpt-5.4-mini batch pricing; image tokens included in prompt_tokens).
    # Input: $0.075/1M (batch); Output: $0.30/1M (batch). Approximate.
    cost_est = (
    ...
    log.info("  Prompt tokens  : %d (includes image tokens)", total_prompt_tokens)
    log.info("  Completion tok : %d", total_completion_tokens)
    log.info("  Cost estimate  : $%.4f (batch discount, rates approximate)", cost_est)
    if failed:
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="VLM image annotation via OpenAI Batch API (submit / status / fetch)"
    )
    sub = p.add_subparsers(dest="command", required=True)

    s = sub.add_parser("submit", help="Download images, build batch JSONL, create batch job")
    ...
    s.add_argument("--image-cache", default=IMAGE_CACHE_DIR,
                   help="Directory to cache preprocessed JPEG images")
    s.add_argument("--model", default=None,
                   help="Override model (default: OPENAI_MODEL_SNAPSHOT from .env)")
    ...
    f = sub.add_parser("fetch", help="Download completed batch and write vlm_annotations.jsonl")
